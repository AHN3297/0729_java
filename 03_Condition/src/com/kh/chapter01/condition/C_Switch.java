...
public class C_Switch {
	 ...
	 * }
	 * switch(정수 || 실수 || 문자 || 문자열){
	 * case 값1 : 실행할코드a; // 동등비교대상 == 값1 true일 경우 a를 수행
	 * case 값2 : 실행할코드b; // 동등비교대상 == 값2 true일 경우 b를 수행
	 * case 값n : 실행할코드n; // 동등비교대상 == 값n true일 경우 n를 수행
	 * 
	 * default : 실행할코드; // == else
	 * }
	 * 
	...
	public void method1() {
		...
		default : System.out.println("메뉴없음");
		}
		...
		default : System.out.println("메뉴없음");
		return;// 현재 실행되고 있는 메소드 영역을아예 빠져 나간다.
		//메소드를 만나면 빠져나오는 부분.
		}
		if (price != 0) {
	...
	public void login() {
		...
		// 1. 회원 가입 시 사용자가 원하는 아이디 값과 비밀번호 값을 입력하여 회원가입을 진행함
