...
public class ServerProgram {
	public static void main(String[] args) {
		 ...
		 * 
		 * ServerSocket
		 * - 포트번호와 연결(Bind)해서 외부의 연결요청을 기다리다가 연결요청이 들어오면수락해줄용도
		 * -수락하는 순간 서버의 Socket객체 생성
		 * 
		 * Socket
		 * - Process간의 통신을 담당
		 * - Socket객체를 이용해서 통신을 위한 기반 스트림 InputStream / OutputStream을 제공받음
		...
		try {
			...
			System.out.println("하염없이기다림...");
			
			// 4) 연결요청이 오면 수락 후 해당 클라이언트와  통신할 ㅅ ㅜ있는 서버 측 Socket객체 생성
			// ServerSocket클래스의 메소드로 accept90 : Socket객체 반환
			...
			// 5) 클라이언트와 통신할 수 있는ㄴ 입/출력용 스트림 생성
			// 6) 보조스트림을 추가해서 성능 개선
			socket.getInputStream();
			br = new BufferedReader(new InputStreamReader(socket.getInputStream()));
			// socket에서 제공하는 입력용 기반 스트립 => InputStream => 1Byte
			// 바이트 스트림과 문자스트림을 호환해줄 수 있는 보조 스트림 => InputStreamReader
			// 속도향상, 한글전달도 뮈한 입력용 보조스트림 => BufferReader => 2Byte
			
			// 출력용 스트림(서버가 클라이언트에게 데이터를 전송하기 위한 스트림
			
			pw = new PrintWriter(socket.getOutputStream());
			// PrintWriter는 2Byte짜리 문자용이지만 !Byte짜리도 시원하게 사용가능
