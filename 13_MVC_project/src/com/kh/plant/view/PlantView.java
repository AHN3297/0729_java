...
public class PlantView {
	...
	private void readAll() {
		...
		//List의 요소가 하나도 존재하지 않을경우
		if(plants.isEmpty()) {
			
			System.out.println("식물이 존재하지않습니다.");
	...
	// 3. 사용자가 식물에 대한 키워드를 입력 할 수 있는 화면을 출력해주는 기능
	private List findByKeyword() {
		...
		if(searched.isEmpty()) {
			...
			System.out.println ("조회결과가 총 " + searched.size() +"건 존재합니다.");
	...
	//5. 특적 식뭏의 정보를 수정할 수 있는 화면을 출력해주ㅡㄴ 기능
	private void updatePlant() {
		...
		// 일치하는 식물을 찾아서
		// 새로운 식무르이 이름 및 타입으로 갱신
		System.out.println("식물 정보 수정 서비스 입니다.");
		//4개 입력
	...
	// 4. 특정식물을 삭제할 수 있는 화면 출력 기능
	private void deletePlant() {
		System.out.println();
		// 삭제를 어떻게 할 수 있도록 만들어 줄 것인가
		// 1. 식물 이름을 입력받아서 삭제
		// 2. 식물 타입을 입력받아서 삭제
		// 3. 식물 이름과 타입을 입력받아서 삭제
		...
		// 3-3) 이름 하고 타입을 입력받고 둘 다 일치하는 값이 있을 때만 삭제
		System.out.println("식물의 이름을 입력해주세요 >");
		String name = sc.nextLine();
		System.out.println("식물의 타입을 입력해주세요 > ");
		String type = sc.nextLine();
