...
public class Run {
	public static void main(String[] args) {
		...
		//1. FieldController 자료형으로fc라는 이름의 지역변수를 선언
		//2. 객체 생성(new키워드를만났을때) global 멤버 변수 할당
		//3. 주소값을 대입
		System.out.println(fc.global);
		fc.checkVariable(50);
		
		//checkVariable호출 종료 시
		...
		*/
		...
		/*
		 * AcessModifier(접근제어자 / 접근제한자)
		 * 
		 * (+)public => 어디서든(같은 패키지, 다른패키지, 클래스 내/외부 포함 전부)접근 가능
		 * 
		 * (#)protected 	=> '같은 패키지'라면 무조건 접근 가능
		 * 				=> 다른 패키지라면 상속구조인 클래스에서만 접근가능
		 * 				=> 상속이라는 개념은 05_Object / 06_ObjectArray 끝나고 07번에서 배운다
		 * 
		 * (~)default(package friendly) => 오로지 같은 패키지에서만 접근가능
		 * 
		 * 
		 * 
