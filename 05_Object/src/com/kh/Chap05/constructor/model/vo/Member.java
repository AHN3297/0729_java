...
public class Member {
	 ...
	 * 		해당 생성자를 통해서 객체 생성 시 실행 하고자하는 코드;
	 * }
	 * 
	 * 생성자의 역할
	...
	//public Member() {
		// 기본생성자(매개변수 없음)
		// System.out.pritln("하핳");
		// 기본 생성자는 객체를 생성할 목적으로 호출해서 사용
	 ...
	 * 3. 매개변수생성자를 명시적으로 작성하면 기본생성자를 만들어주지않음!
	 *  => 우리는 꼭 기본생성자를 억지로 열심히 작성하는 습관을 들여야 함!!
	 *  4. 생성자는 동일한 이름으로 여러 개 생성이 간으하지만 매개변수가 중복되어선 안된다. => 생성자 오버로딩
	 */
	/*
	 * 2025 / 08 / 07
	 * 기존에 만들어 두었던 다섯 개의 VO클래스 및 실행 클래스를
	 * 오늘 배운 캡슐화 및 생성자를 적용하여 변경 하시오.
	 * 
	 *   1) 모든필드의 접근 제한자를 private으로 변경
	 *   2) 필드에 간접적으로 접근할 수있도록 getter / setter 생성
	 *   3) 모든 필드의 정보를 합쳐서 문자열의 형태로 반환해주는 info()생성
	 *   4) 기본 생성자 및 모든 필드를 매개변수로 가지고 있는 생성자 생성
	 *   5) 직접접근으로 사용하고 있던 실행 클래스의 코드들을 전부 다 getter / setter대체
	 * 
	 * 
	 * 
	 */
	...
	// [메소드부]
	// 16:00 실습
	// 모든 필드에  대한 getter() / setter()만들기 + 모든 필드의 정보를 합쳐서 반환하는 info만들기
