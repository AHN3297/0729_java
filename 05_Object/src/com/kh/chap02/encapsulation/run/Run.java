...
public class Run {
	// 캡슐화 (encapsulation)
	// 속성(데이터)과 해당 데이터를 조작하는 행동(메소드를 하나의 단위로 묶는 것)
	// 정보은닉(information hiding)
	
	
	
	//접근 제한자 (예약어) 반환혀 메소드  식별자(매개변수){   }
	
	public static void main(String[] args) {
		
		//변수 선언    대입연산자      객체 생성 == 메모리적재
		Cafe cafe = new Cafe();
		/*
		cafe.name = "KH카페";
		...
		*/
		//필드가 보이지 않아서(외부에서 ㅓㅈㅂ근이 불가능 해서) 오류 발생
		// 접근 제한자를 public에서private으로 변셩했기 때문에 직접 접근이 불가능
		// 직접 접근이 불가능하기 때문에 간접적으로 접근할 수 있는 방법을 만들어주어야함
		// Cafe 라는 클래스에 getter / setter라는 메소드 만들기
		...
		cafe.setSignatureMenuPrice(500);
								 //인자값
		cafe.setName("KH카페");
		cafe.setAddress("서울시 중구 남대문로 120 층");
